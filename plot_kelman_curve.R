##############################################################
# Plot Kelman Curve From Blood Gas Datasets
# Author: Wanjun Gu
...
plot_kelman_curve = function(sat  = c(90.3,85.2,83.8,84.4,41.5),
  ...
  deviation_p50 = 26.8 - p50
  
  virtual_saturation = vector()
  n = 1
  for(virtual_po2 in seq(1,100,1)){
    virtual_saturation[n] = get_saturation(deviation_p50 = deviation_p50,
                                           po2 = virtual_po2,
                                           temp = 37,
                                           ph = 7.4,
                                           pco2 = 40)
  ...
  plot(seq(1,100,1),
       virtual_saturation, 
       type = "l",
       main = "Kelman Dissociation Curve",
       sub = paste0("P50 = ", p50),
       xlab = "Partial Pressure of Oygen",
       ylab = "Oxygen Saturation in Hemoglobin")
  points(po2, sat, pch = 1)
  qualified_data_index = which(sat >= lower_limit & sat <= upper_limit)
  sat = sat[qualified_data_index]
  po2 = po2[qualified_data_index]
  temp = temp[qualified_data_index]
